...
void updateCameras()
{
  for (ir::CameraPtr camera : g_cameras)
  {
    if (g_cameraPos == 0u)
    {
      ...
      camera->Scene()->LightByName("sun")->SetDiffuseColor(1.0, 0.8, 0.5);
      camera->Scene()->LightByName("sun")->SetSpecularColor(0.5, 0.5, 0.5);
    }
    else if (g_cameraPos == 1u)
    {
      camera->SetLocalPosition(0, 0.0, 2.0);
      camera->SetLocalRotation(0.0, 0.0, 0.0);

      camera->Scene()->LightByName("sun")->SetDiffuseColor(0.0, 0.0, 0.0);
      camera->Scene()->LightByName("sun")->SetSpecularColor(0.0, 0.0, 0.0);
    }
    else if (g_cameraPos == 2u)
    {
      camera->SetLocalPosition(12, -1.8, 2.0);
      camera->SetLocalRotation(0.0, 0.2, -0.1);
